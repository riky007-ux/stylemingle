...
export async function POST(req: Request) {
  ...
  const prefs = await db.select({ rating: ratings.rating }).from(ratings).where(eq(ratings.userId, userId));
  let avgRating = 0;
  if (prefs.length > 0) {
  ...
  const prompt = `Generate a stylish outfit description for wardrobe item IDs: ${itemIds.join(', ')}. ${preferenceNote}`;
  let description = '';
  try {
  ...
  const result = await db.insert(outfits).values({ id, userId, createdAt: Date.now() }).returning();
